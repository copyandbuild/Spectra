...
class Moderation(commands.Cog):
    ...
    async def ban(self, ctx, user: Option(str, description="› Der Nutzer (ID oder Username), den du bannen möchtest"),
                  grund: str = Option(autocomplete=reasons,
                                      description="Grund für den Ban",
                                      default="Kein Grund angegeben")):
        try:
            if ctx.author.guild_permissions.ban_members:
                # Check if the user is an ID
                if user.isdigit():
                    user_id = int(user)
                    try:
                        user_obj = await self.bot.fetch_user(user_id)
                        await ctx.guild.ban(discord.Object(id=user_id), reason=grund)
                        await ctx.respond(f'Banned **{user_obj.name}**')
                    except discord.NotFound:
                        await ctx.respond(f'unknown user.', ephemeral=True)
                else:
                    await ctx.respond(f'Benutzer {user} nicht gefunden.', ephemeral=True)
            else:
    ...
    @mod.command(description="› Kicke einen Nutzer")
    async def kick(self, ctx, user: Option(discord.Member, description="› Der Nutzer, den du kicken möchtest"),
                  grund: str = Option(autocomplete=reasons,
                                      description="Grund für den Kick",
                                      default="Kein Grund angegeben")):
        try:
            if ctx.author.guild_permissions.kick_members:
                await user.send(f"> Du wurdest von `{ctx.guild.name}` gekickt! \n> Grund: {grund}")
                ...
                embed.add_field(name="Kick Grund", value=f"```yml\n{grund}\n```", inline=True)
                embed.add_field(name="Verantwortlicher Moderator", value=f"```yml\n{ctx.author.name}\n```", inline=True)
                await ctx.respond(embed=embed)
            else:
                raise discord.Forbidden("Du hast nicht die Berechtigung, Mitglieder zu kicken.")
        except discord.Forbidden as e:
            embed = discord.Embed(
                title=":warning: NUTZER ZU HOCH oder GLEICHE ROLLE :warning:",
                description="**Info for Devs:\n*" + str(e) + "*",
